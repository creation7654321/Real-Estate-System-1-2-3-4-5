...
class Event {
	...
	public function get_title() {
		...
		return sprintf( esc_html__( 'Event #%d', 'easy-wp-smtp' ), $this->get_id() );
	}
	...
	public static function get_types() {
		...
		return [
			self::TYPE_ERROR => esc_html__( 'Error', 'easy-wp-smtp' ),
			self::TYPE_DEBUG => esc_html__( 'Debug', 'easy-wp-smtp' ),
		];
	}
	...
	public function get_created_at_formatted() {
		...
		if ( empty( $date ) ) {
			return esc_html__( 'N/A', 'easy-wp-smtp' );
		}
	...
	/**
	 * Get the event's initiator type.
	 *
	 * @since 2.0.0
	 *
	 * @return string
	 */
	private function get_initiator_type() {
		...
		$initiator = WP::get_initiator( $initiator['file'] );
	...
	public function get_details_html() {
		...
		$initiator_type      = $this->get_initiator_type();
		$initiator_backtrace = $this->get_initiator_backtrace();

		ob_start();
		?>
		<div class="easy-wp-smtp-debug-event-preview">
			<div class="easy-wp-smtp-debug-event-preview-subtitle">
				<span><?php esc_html_e( 'Debug Event Details', 'easy-wp-smtp' ); ?></span>
			</div>
			<div class="easy-wp-smtp-debug-event-preview-table">
				<div class="easy-wp-smtp-debug-event-row easy-wp-smtp-debug-event-preview-type">
					<span class="debug-event-label"><?php esc_html_e( 'Type', 'easy-wp-smtp' ); ?></span>
					<span class="debug-event-value"><?php echo esc_html( $this->get_type_name() ); ?></span>
				</div>
				<div class="easy-wp-smtp-debug-event-row easy-wp-smtp-debug-event-preview-date">
					<span class="debug-event-label"><?php esc_html_e( 'Date', 'easy-wp-smtp' ); ?></span>
					<span class="debug-event-value"><?php echo esc_html( $this->get_created_at_formatted() ); ?></span>
				</div>
				<div class="easy-wp-smtp-debug-event-row easy-wp-smtp-debug-event-preview-content">
					<span class="debug-event-label"><?php esc_html_e( 'Content', 'easy-wp-smtp' ); ?></span>
					<div class="debug-event-value">
						<?php echo wp_kses( str_replace( [ "\r\n", "\r", "\n" ], '<br>', $this->get_content() ), [ 'br' => [] ] ); ?>
					</div>
				</div>
				<?php if ( ! empty( $initiator ) ) : ?>
					<div class="easy-wp-smtp-debug-event-row easy-wp-smtp-debug-event-preview-caller">
						<span class="debug-event-label"><?php esc_html_e( 'Caller', 'easy-wp-smtp' ); ?></span>
						<div class="debug-event-value">
							<span class="debug-event-initiator">
								<?php
								if ( $initiator_type === 'plugin' || $initiator_type === 'mu-plugin' ) {
									printf( /* translators: %s - caller plugin name. */
										esc_html__( 'Plugin: %s', 'easy-wp-smtp' ),
										esc_html( $initiator )
									);
								} else if ( $initiator_type === 'theme' ) {
									printf( /* translators: %s - caller theme name. */
										esc_html__( 'Theme: %s', 'easy-wp-smtp' ),
										esc_html( $initiator )
									);
								} else {
									echo esc_html( $initiator );
								}
								?>
							</span>
							<p class="debug-event-code">
								...
								printf( /* Translators: %1$s the path of a file, %2$s the line number in the file. */
									esc_html__( '%1$s (line: %2$s)', 'easy-wp-smtp' ),
	...
	public function get_short_details() {
		...
		if ( ! empty( $this->get_initiator() ) ) {
			$result[] = sprintf(
				...
				esc_html__( 'Email Source: %s', 'easy-wp-smtp' ),
