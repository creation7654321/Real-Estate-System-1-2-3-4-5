...
class DomainChecker {
	 ...
	 * @since 2.1.0
	 */
	const ENDPOINT = 'https://connect.easywpsmtp.com/domain-check/';
		...
		if ( is_wp_error( $response ) ) {
			$this->results = [
				...
				'message' => method_exists( $response, 'get_error_message' ) ?
					...
					esc_html__( 'Something went wrong. Please try again later.', 'easy-wp-smtp' ),
				'checks'  => [],
			];
		} else {
	...
	public function is_supported_mailer() {
		...
		return ! in_array( $this->mailer, [ 'mail' ], true );
	}
	...
	public function get_results_html() {
		...
		ob_start();
		?>
		<div id="easy-wp-smtp-domain-check-details">
			...
			<?php if ( ! empty( $results['checks'] ) ) : ?>
				<div class="easy-wp-smtp-domain-check-details-check-list">
					<?php foreach ( $results['checks'] as $check ) : ?>
						<div class="easy-wp-smtp-domain-check-details-check-list-item">
							<img src="<?php echo esc_url( easy_wp_smtp()->assets_url . '/images/icons/' . esc_attr( isset( $icons[ $check['state'] ] ) ? $icons[ $check['state'] ] : 'exclamation-triangle-orange' ) . '.svg' ); ?>" class="easy-wp-smtp-domain-check-details-check-list-item-icon" alt="<?php printf( /* translators: %s - item state name. */ esc_attr__( '%s icon', 'easy-wp-smtp' ), esc_attr( $check['state'] ) ); ?>">
							<div class="easy-wp-smtp-domain-check-details-check-list-item-content">
