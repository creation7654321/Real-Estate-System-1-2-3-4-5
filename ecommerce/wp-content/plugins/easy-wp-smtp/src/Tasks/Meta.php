 ...
 * between Action Scheduler and Easy WP SMTP hooks arguments.
...
class Meta {
	...
	public static function get_table_name() {
		...
		return $wpdb->prefix . 'easywpsmtp_tasks_meta';
	}
	...
	private function add_to_db( $data, $type = '' ) {
		...
		do_action( 'easy_wp_smtp_pre_insert_' . $type, $data );
		...
		do_action( 'easy_wp_smtp_post_insert_' . $type, $wpdb->insert_id, $data );
	...
	public function update( $row_id, $data = array(), $where = '', $type = '' ) {
		...
		do_action( 'easy_wp_smtp_pre_update_' . $type, $data );
		...
		do_action( 'easy_wp_smtp_post_update_' . $type, $data );
	...
	public function delete( $row_id = 0 ) {
		...
		do_action( 'easy_wp_smtp_pre_delete', $row_id );
		do_action( 'easy_wp_smtp_pre_delete_' . $this->type, $row_id );
		...
		do_action( 'easy_wp_smtp_post_delete', $row_id );
		do_action( 'easy_wp_smtp_post_delete_' . $this->type, $row_id );
	...
	public function delete_by( $column, $column_value ) {
		...
		do_action( 'easy_wp_smtp_pre_delete', $column_value );
		do_action( 'easy_wp_smtp_pre_delete_' . $this->type, $column_value );
		...
		do_action( 'easy_wp_smtp_post_delete', $column_value );
		do_action( 'easy_wp_smtp_post_delete_' . $this->type, $column_value );
