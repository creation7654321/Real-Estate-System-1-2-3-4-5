...
class Tasks {
	 ...
	 * @since 2.0.0
	 */
	const GROUP = 'easy_wp_smtp';

	/**
	 * Easy WP SMTP pending or in-progress actions.
	 *
	 * @since 2.0.0
	 *
	 * @var array
	...
	public function init() { // phpcs:ignore WPForms.PHP.HooksMethod.InvalidPlaceForAddingHooks
		...
		foreach ( $this->get_tasks() as $task ) {
			if ( ! is_subclass_of( $task, '\EasyWPSMTP\Tasks\Task' ) ) {
				continue;
			}
	...
	public function get_tasks() {
		 ...
		 */
		return apply_filters( 'easy_wp_smtp_tasks_get_tasks', $tasks );
	}
	...
	/**
	 * Get all Easy WP SMTP pending or in-progress actions.
	 *
	 * @since 2.0.0
	 */
