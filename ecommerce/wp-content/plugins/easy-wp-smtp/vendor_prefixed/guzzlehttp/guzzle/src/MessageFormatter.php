...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\EasyWPSMTP\Vendor\Psr\Http\Message\RequestInterface $request, \EasyWPSMTP\Vendor\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \EasyWPSMTP\Vendor\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \EasyWPSMTP\Vendor\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
     ...
     * @return string
     */
    private function headers(\EasyWPSMTP\Vendor\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
