...
use EasyWPSMTP\Vendor\PhpAmqpLib\Message\AMQPMessage;
use EasyWPSMTP\Vendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
class AmqpHandler extends \EasyWPSMTP\Vendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, $exchangeName = 'log', $level = \EasyWPSMTP\Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if ($exchange instanceof \AMQPExchange) {
            $exchange->setName($exchangeName);
        } elseif ($exchange instanceof \EasyWPSMTP\Vendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    {
        return new \EasyWPSMTP\Vendor\PhpAmqpLib\Message\AMQPMessage((string) $data, array('delivery_mode' => 2, 'content_type' => 'application/json'));
    }
    /**
     * {@inheritDoc}
     */
