...
use Exception;
use EasyWPSMTP\Vendor\Monolog\Formatter\LineFormatter;
use EasyWPSMTP\Vendor\Monolog\Logger;
use EasyWPSMTP\Vendor\Monolog\Utils;
use EasyWPSMTP\Vendor\PhpConsole\Connector;
use EasyWPSMTP\Vendor\PhpConsole\Handler;
use EasyWPSMTP\Vendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \EasyWPSMTP\Vendor\Monolog\Handler\AbstractProcessingHandler
{
    private $options = array(
        'enabled' => \true,
        // bool Is PHP Console server enabled
        'classesPartialsTraceIgnore' => array('EasyWPSMTP\\Vendor\\Monolog\\'),
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \EasyWPSMTP\Vendor\PhpConsole\Connector $connector = null, $level = \EasyWPSMTP\Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('EasyWPSMTP\\Vendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\EasyWPSMTP\Vendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \EasyWPSMTP\Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \EasyWPSMTP\Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\EasyWPSMTP\Vendor\PhpConsole\Helper::isRegistered()) {
            \EasyWPSMTP\Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \EasyWPSMTP\Vendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \EasyWPSMTP\Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \EasyWPSMTP\Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \EasyWPSMTP\Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
