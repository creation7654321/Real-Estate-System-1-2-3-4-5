...
use EasyWPSMTP\Vendor\Monolog\Handler\HandlerInterface;
use EasyWPSMTP\Vendor\Monolog\Handler\StreamHandler;
use EasyWPSMTP\Vendor\Psr\Log\LoggerInterface;
use EasyWPSMTP\Vendor\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \EasyWPSMTP\Vendor\Psr\Log\LoggerInterface, \EasyWPSMTP\Vendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\EasyWPSMTP\Vendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \EasyWPSMTP\Vendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \EasyWPSMTP\Vendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \EasyWPSMTP\Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \EasyWPSMTP\Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
